// url path regex: [boardName]/(thread/)?threadNumber/?(((word|-)+)?(/?#?(p|q)?postNumber)?)?
const urlPathRegex = /\/\w+\/(thread\/)?\d+(\/)?(((\w+|-)+)?((\/)?#?(p|q)?\d+)?)?/g;
...
const sitesToReplaceRegex = new RegExp("https?:\/\/(www\.)?(" + chanRegex.source + "|" + yukiRegex.source + "|" + nyafuuRegex.source + "|" + moeArchiveRegex.source + "|" + bStatsRegex.source + "|" + desuStorageRegex.source + ")" + urlPathRegex.source);
