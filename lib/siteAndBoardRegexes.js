// TODO: simplify this
// link end regex: [boardName]/(thread/?)threadNumber(/)?(((word | -)+)? (#(p|q)?postNumber)?)?
const chanRegex = /boards(?:\.(4channel|4chan)\.org)\/\w+\/(\w+\/)?\d+(\/)?(((\w+|-)+)?((\/)?#?(p|q)?\d+)?)?/g; // includes #postNum and #word 
const yukiRegex =  /(?:yuki\.la)\/\w+\/(\w+\/)?\d+(\/)?(((\w+|-)+)?((\/)?#?(p|q)?\d+)?)?/g;
const nyafuuRegex = /archive\.nyafuu\.org\/\w+\/(\w+\/)?\d+(\/)?(((\w+|-)+)?((\/)?#?(p|q)?\d+)?)?/g;
const moeArchiveRegex = /archive\.moe\.org\/\w+\/(\w+\/)?\d+(\/)?(((\w+|-)+)?((\/)?#?(p|q)?\d+)?)?/g;
const bStatsRegex = /archive\.b-stats\.org\/\w+\/(\w+\/)?\d+(\/)?(((\w+|-)+)?((\/)?#?(p|q)?\d+)?)?/g;
const desuStorageRegex = /desu-storage\.org\/\w+\/(\w+\/)?\d+(\/)?(((\w+|-)+)?((\/)?#?(p|q)?\d+)?)?/g;

const sitesToReplace = [chanRegex, yukiRegex, nyafuuRegex, moeArchiveRegex, bStatsRegex, desuStorageRegex];

const sitesToReplaceRegex = new RegExp("https?:\/\/(www\.)?(" + chanRegex.source + "|" + yukiRegex.source  + "|" + bStatsRegex.source + nyafuuRegex.source + "|" + moeArchiveRegex.source + "|" + desuStorageRegex.source + ")");
