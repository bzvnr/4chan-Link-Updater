const chanRegex = /boards(?:\.(4channel|4chan)\.org)\/\w+\/\w+\/\d+(\/\w+|#p\d+)?/g; // includes #postNum and #word 
const yukiRegex =  /(?:yuki\.la)\/\w+\/\d+(#(p|q)?\d+)?/g;
const nyafuuRegex = /archive\.nyafuu\.org\/\w+\/\w+\/\d+(\/\w+|#p\d+)?/g;
const moeArchiveRegex = /archive\.moe\.org\/\w+\/\w+\/\d+(\/\w+|#p\d+)?/g;
const bStatsRegex = /archive\.b-stats\.org\/\w+\/\w+\/\d+(\/\w+|#p\d+)?/g;
const desuStorageRegex = /desu-storage\.org\/\w+\/\w+\/\d+(\/\w+|#p\d+)?/g;;
...
const liveSitesAndBoards = {
  ...
  "thebarchive.com": /\/b\//,
  "warosu.org": /\/(3|biz|cgl|ck|diy|fa|g|ic|jp|lit|sci|tg|vr|vt)\//,
}
